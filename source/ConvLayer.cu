...
ConvLayer::ConvLayer(const std::string& w_path, int pad, int stride, bool bias, bool Relu, bool extra_down):
    _pad(pad),
    _stride(stride),
    _bias(bias),
    input_set(false),
...
void ConvLayer::forward() 
{
    ...
    rmgemm(m, n, k, _wcol->_ptr, _imcol->_ptr, _res->_ptr);
...
void ConvLayer::set_input(std::shared_ptr<Tensor<float>> input)
{
    ...
    _imcol2 = std::shared_ptr<Tensor<float>>(new Tensor<float>({C*H*W, batch_size*Ho*Wo}));
