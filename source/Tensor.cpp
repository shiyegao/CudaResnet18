...
{
    int newcount = 1;
    for (int i = 0; i < int(newsize.size()); ++i) {
        newcount *= newsize[i];
    }
    if (newcount != _count) {
...
{
    cuda_relu(this->_ptr, this->_ptr, this->count());
    return *this;
}

template<typename T>
Tensor<T>& Tensor<T>::add_relu(const Tensor<T>& src2)
{
    if (this->count() != src2.count()){
        throw std::runtime_error("different size in Tensor::add_inplace");
    }
    cuda_add_relu(this->_ptr, src2._ptr, this->_ptr, this->count());
    return *this;
}
